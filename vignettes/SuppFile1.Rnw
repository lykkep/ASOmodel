...
nu['OT',c('kOT*OT','kOTE*OT*E','vdegrad*OT')] <- -1
#OTE
nu['OTE',c('kOTpE*OT*E')] <- 1
nu['OTE',c('kOTE*OTE','vdegrad*OTE','kcleav*OTE')] <- -1
#E
nu['E',c('kOTE*OTE','vdedrag*OTE','kOTE*OCE')] <- 1
nu['E',c('kOTpE*OT*E')] <- -1
#O
...
plot(Gillespie$data[,1],rowSums(Gillespie$data[,2:4])/(parms1["vprod"]/parms1["vdedrag"]),
...
@
<<IC50,echo=F,fig=TRUE,width=7,height=4,include=FALSE>>=
...
LegendS=as.expression(sapply(signif(KM[seq(1,11,by=2)],2),
