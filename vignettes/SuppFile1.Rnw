...
$v_{prod}$ & Production of target & 0.2 nM/min & \cite{lodish2008molecular}\\
$\vd$ & Degradation of target & 0.04 $\rm{min}^{-1}$ &  \cite{Yang:2003ja}\\
$K_{dOT}$ & Dissociation constant of $OT$ & 0.3 nM &  \cite{Christensen:2001te} \\
$K_{dOTE}$ & Dissociation constant of $OTE$  & 70 nM &  \cite{Amirkhanov:2002vo} \\
$k_{O+T \to OT }$ & Rate of $O+T \to OT$ & $0.2 \, (\rm{nM \,min})^{-1}$ &  \cite{Christensen:2001te}\\
$k_{OT+E \to OTE}$ & Rate of $OT+E \to OTE$  & 5 $(\rm{nM \,min})^{-1}$ &  \cite{Amirkhanov:2002vo}\\
$k_{OTE \to OCE}$ & Rate of cleavage  & 8 $\rm{min}^{-1}$ & \cite{Amirkhanov:2002vo}\\
$\alpha$ & Ratio of $\frac{k_{OT \to O+T}}{k_{\mathrm{OC \to O+C}}} \le 1$ & 0.1  & {}\\
\hline
\end{tabular}
\end{table}
%%%%
...
\caption{Dose-response curves for different values of $E_{t}$, $\alpha$, $\vp$, $\vd$, $K_{dOT}$, $\kOpT$, $K_{dOTE}$, $\kt$, and $\kE$ (top,left to bottom,right). Black lines correspond to the parameter values listed in Supplementary Table \ref{tb::par}.}\label{fig::Etot}
\end{figure}
...
@
For a range of $K_{dOT}$ values, the corresponding $\EC$-values can be calculated. These can be fitted to a second order polynomial by using the R-function \texttt{lm()}, see Supplementary Figure S2. 
<<>>==
...
\end{Ncenter}
\caption{$\EC$ as a function of $\kdOT$ is fitted on a log-log scale to a second order polynomial.}\label{fig::Opt}
\end{figure}
